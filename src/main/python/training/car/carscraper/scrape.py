...
def all_makes():
    all_makes_list = []
    for a in fetch(website, "/new-cars").find_all("div", {"class": "menu-column"}):
        for b in a.find_all("a"):  
            all_makes_list.append(b['href'])
    return all_makes_list

def make_menu(listed):
    make_menu_list = []
    for make in listed: 
        for div in fetch(website, make).find_all("div", {"class": "make"}):
            make_menu_list.append(div.find_all("div", {"class": "info"})[0].find_all("a")[0]['href'])
    return make_menu_list


def model_menu(listed):
    model_menu_list = []
    for make in listed[0:10]:
        soup = fetch(website, make)
        for div in soup.find_all("div", {"class": "year-selector"}):
            for link in div.find_all("a"):
                model_menu_list.append(link['href'])
...
def specs_and_pics(listed):
    ...
    for row, pic in zip(listed, picture_tab):
        ...
        try:
            ...
            specifications_df.loc['Year', :] = soup.find_all('h1', {'id': 'title'})[0].text.strip().split(' ')[0]
        except:
